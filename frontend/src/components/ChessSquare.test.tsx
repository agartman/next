...
describe('ChessSquare', () => {
  ...
  it('renders piece when provided', () => {
    ...
    renderWithTheme(<ChessSquare {...defaultProps} piece={piece} />);
  ...
  it('handles click events', () => {
    const onClick = vi.fn();
    renderWithTheme(<ChessSquare {...defaultProps} onClick={onClick} />);
  ...
  it('does not handle clicks when disabled', () => {
    const onClick = vi.fn();
    renderWithTheme(<ChessSquare {...defaultProps} onClick={onClick} disabled={true} />);
  ...
  it('shows file coordinate on bottom row squares', () => {
    renderWithTheme(<ChessSquare {...defaultProps} square="e1" />);

    expect(screen.getByText('e')).toBeInTheDocument();
  });

  it('shows rank coordinate on left column squares', () => {
    renderWithTheme(<ChessSquare {...defaultProps} square="a4" />);

    expect(screen.getByText('4')).toBeInTheDocument();
  });

  it('shows both coordinates on corner squares', () => {
    renderWithTheme(<ChessSquare {...defaultProps} square="a1" />);
  ...
  it('does not show coordinates on middle squares', () => {
    renderWithTheme(<ChessSquare {...defaultProps} square="d4" />);
  ...
  it('renders different piece types correctly', () => {
    ...
    pieces.forEach((piece, index) => {
      const { unmount } = renderWithTheme(
        <ChessSquare {...defaultProps} square={`e${index + 1}`} piece={piece} />
      );
  ...
  it('applies correct styling classes based on props', () => {
    // Test selected state
    const { rerender } = renderWithTheme(<ChessSquare {...defaultProps} isSelected={true} />);
    ...
    rerender(
      <ThemeProvider theme={theme}>
        <ChessSquare {...defaultProps} isValidMove={true} />
      </ThemeProvider>
    );
    ...
    rerender(
      <ThemeProvider theme={theme}>
        <ChessSquare {...defaultProps} isCheck={true} />
      </ThemeProvider>
    );
