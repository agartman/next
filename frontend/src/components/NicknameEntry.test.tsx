...
describe('NicknameEntry', () => {
    ...
    it('validates nickname input correctly', async () => {
        ...
        await user.click(submitButton);
        expect(screen.getByText(/nickname must be no more than 20 characters long/i)).toBeInTheDocument();
        expect(mockOnSubmit).not.toHaveBeenCalled();
        ...
        await user.click(submitButton);
        expect(screen.getByText(/nickname can only contain letters, numbers, underscores, and hyphens/i)).toBeInTheDocument();
        expect(mockOnSubmit).not.toHaveBeenCalled();
    });
