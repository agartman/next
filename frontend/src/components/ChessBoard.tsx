...
const StatusMessage = styled.div<{ $type: 'check' | 'checkmate' | 'stalemate' | 'draw' | 'normal' }>`
  padding: ${({ theme }) => theme.spacing.sm} ${({ theme }) => theme.spacing.md};
  border-radius: 4px;
  font-weight: ${({ theme }) => theme.typography.fontWeight.medium};
  background-color: ${({ theme, $type }) => {
    switch ($type) {
      case 'check': return theme.colors.status.warning;
      case 'checkmate': return theme.colors.status.error;
      case 'stalemate':
      case 'draw': return theme.colors.status.info;
