  ...
  private handleJoinRoom(socket: TypedSocket, data: { roomId: string; password: string }): void {
    try {
      ...
      if (!joinResult.success) {
        const errorCode =
          joinResult.error === 'Room not found'
            ? 'ROOM_NOT_FOUND'
            : joinResult.error === 'Incorrect password'
              ? 'WRONG_PASSWORD'
              : joinResult.error === 'Room is full'
                ? 'ROOM_FULL'
                : 'VALIDATION_ERROR';
    ...
    try {
      ...
      if (whitePlayer && blackPlayer) {
        this.gameEngine.initializeGame(
          session.currentRoom,
          whitePlayer.sessionId,
          blackPlayer.sessionId
        );
        const gameState = this.gameEngine.getGameState(session.currentRoom);
